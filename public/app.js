...
let state = {
  ...
  favorites: LS.get("spt_favorites", []),        // [productId,...]
  address: LS.get("spt_address", { city:"Москва", street:"", house:"", block:"", flat:"", entrance:"", floor:"", intercom:"", comment:"" }),
  profile: LS.get("spt_profile", { phone:"", name:"", loggedIn:false, bonuses: 150 }),
  orderIds: LS.get("spt_order_ids", []),         // [ "SPT-123456", ... ] — источник правды это сервер
  ordersCache: {},                               // id -> заказ (заполняется через API при просмотре)
  promo: LS.get("spt_promo", null),              // {code, type, value, minSum, discount} — из ответа /api/promo/validate
  fulfillment: LS.get("spt_fulfillment", "delivery"), // delivery | pickup
};

function saveCart(){ LS.set("spt_cart", state.cart); renderBadges(); }
function saveFavorites(){ LS.set("spt_favorites", state.favorites); renderBadges(); }
function saveAddress(){ LS.set("spt_address", state.address); }
function saveProfile(){ LS.set("spt_profile", state.profile); }
function saveOrderIds(){ LS.set("spt_order_ids", state.orderIds); }
...
function toggleFavorite(id){
  ...
  else { state.favorites.push(id); toast("Добавлено в избранное", "success"); }
  saveFavorites();
...
function renderApp(){
  ...
  switch(view){
    ...
    case "profile": app.innerHTML = viewProfile(); break;
  ...
  <article class="product-card" data-action="open-product" data-id="${p.id}">
    ...
    <div class="pc-body">
      ...
      <div class="pc-bottom">
        ...
        <button class="pc-add" data-action="quick-add" data-id="${p.id}">Добавить</button>
      </div>
    </div>
  </article>`;
}
...
function viewHome(){
  ...
  <section class="hero">
    <div class="hero-inner">
      <div class="hero-copy">
        <span class="eyebrow">Москва и Московская область · от 35 минут</span>
        <h1>Суши и пицца<br>с&nbsp;доставкой</h1>
        <p class="hero-sub">Свежие роллы, аппетитная пицца и любимые блюда — быстро доставим прямо к вам.</p>
        <div class="hero-actions">
          <button class="btn btn-primary btn-lg" data-action="go-menu">Заказать сейчас</button>
          <button class="btn btn-ghost btn-lg" data-action="go-menu">Посмотреть меню</button>
        </div>
        <ul class="hero-perks">
          <li>🚀 Быстрая доставка</li>
          <li>🌿 Свежие продукты</li>
          <li>📖 Большой выбор</li>
          <li>🏷️ Выгодные акции</li>
        </ul>
      </div>
      <div class="hero-visual" aria-hidden="true">
        ...
        <div class="hero-ticket">
          ...
          <div class="ticket-row ticket-total"><span>Итого</span><span>1 448 ₽</span></div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section promo-strip">
    <div class="section-head">
      <h2>Акции</h2>
      <button class="link-more" data-action="go" data-view="promotions">Все акции →</button>
    </div>
  ...
  <section class="section">
    <div class="section-head">
      <h2>🔥 Популярное</h2>
      <button class="link-more" data-action="go-category" data-cat="popular">Всё меню →</button>
    </div>
    ${productGrid(popular)}
  </section>
  ...
  <section class="section">
    <div class="section-head">
      <h2>✨ Новинки</h2>
      <button class="link-more" data-action="go-category" data-cat="new">Всё меню →</button>
    </div>
    ${productGrid(isNew)}
  </section>

  <section class="section categories-showcase">
    <div class="section-head"><h2>Категории меню</h2></div>
  ...
  <section class="section delivery-teaser">
    <div class="dt-card">
      <div>
        <h2>Доставим быстро и бережно</h2>
        <p>Бесплатная доставка в пределах МКАД при заказе от 1500 ₽. Средний срок — 35–55 минут.</p>
        <button class="btn btn-primary" data-action="go" data-view="delivery">Условия доставки</button>
      </div>
...
function renderProductModal(p){
  ...
  modal.innerHTML = `
    ...
    <div class="pm-body">
      ...
      <div class="pm-facts">
        <div><span>Состав</span><strong>${p.ingredients}</strong></div>
        <div><span>Вес</span><strong>${p.weight}</strong></div>
        ${p.pieces ? `<div><span>Количество</span><strong>${p.pieces} шт.</strong></div>` : ''}
        <div><span>Аллергены</span><strong>может содержать сою, глютен, кунжут</strong></div>
      </div>
  ...
  modal.innerHTML = `
    ...
    <div class="promo-modal-body">
      ...
      ${promo.code ? `
        ...
        <button class="btn btn-primary btn-lg" data-action="use-promo" data-code="${promo.code}">Применить и перейти в меню</button>
      ` : `<button class="btn btn-primary btn-lg" data-action="go-menu">Перейти в меню</button>`}
    </div>
  `;
}
...
function viewFavorites(){
  ...
  return `
  <section class="section">
    <div class="section-head"><h2>❤️ Мои любимые блюда</h2></div>
    ${items.length ? productGrid(items) : `
      <div class="empty-state">
        <span class="empty-emoji">🤍</span>
        <h3>Пока ничего нет в избранном</h3>
        <p>Нажимайте на сердечко на карточке блюда, чтобы сохранить его сюда.</p>
        <button class="btn btn-primary" data-action="go-menu">Перейти в меню</button>
      </div>`}
  </section>`;
}
...
function cartSummaryMarkup(showCheckoutBtn){
  ...
  return `
    ...
    <div class="promo-input-row">
      <input type="text" id="promoInput" placeholder="Промокод" value="${state.promo ? state.promo.code : ''}">
      <button class="btn btn-ghost" data-action="apply-promo">Применить</button>
    </div>
    <div id="promoError" class="field-error"></div>
    <div class="cart-lines">
      <div class="cart-line"><span>Товары</span><span>${money(sub)}</span></div>
      ${disc>0 ? `<div class="cart-line cart-line-discount"><span>Скидка${state.promo? ' ('+state.promo.code+')':''}</span><span>−${money(disc)}</span></div>` : ""}
      <div class="cart-line"><span>Доставка</span><span>${del===0 ? "Бесплатно" : money(del)}</span></div>
    </div>
    <div class="cart-total-row">
      <span>Итого</span><span>${money(total)}</span>
    </div>
    ${showCheckoutBtn ? `<button class="btn btn-primary btn-lg btn-block" data-action="go-checkout">Оформить заказ</button>` : ""}
  `;
}
  ...
  if (state.cart.length===0){
    body.innerHTML = `
      <div class="empty-state">
        ...
        <button class="btn btn-primary" data-action="go-menu-close-cart">Перейти в меню</button>
      </div>`;
    foot.innerHTML = "";
    return;
  }
...
function renderAddressModal(){
  ...
  modal.innerHTML = `
    ...
    <p class="modal-sub">Мы подберём ближайший ресторан и рассчитаем время доставки.</p>
    <div class="city-grid">
    ...
    <button class="btn btn-primary btn-lg btn-block" data-action="save-quick-address">Сохранить адрес</button>
  `;
}
...
const CHECKOUT_STEPS = ["Получение","Адрес","Контакты","Оплата","Подтверждение"];
...
function viewCheckout(){
  if (state.cart.length===0){
    return `<section class="section"><div class="empty-state"><span class="empty-emoji">🛒</span><h3>Корзина пуста</h3><p>Добавьте товары, чтобы оформить заказ.</p><button class="btn btn-primary" data-action="go-menu">Перейти в меню</button></div></section>`;
  }
    ...
    <div class="checkout-layout">
      <div class="checkout-form">
        ...
        <div class="checkout-nav">
          ${checkoutStep>1 ? `<button class="btn btn-ghost" data-action="checkout-back">Назад</button>` : `<span></span>`}
          ${checkoutStep<5 ? `<button class="btn btn-primary" data-action="checkout-next">Продолжить</button>` : `<button class="btn btn-primary btn-lg" data-action="checkout-confirm" ${orderSubmitting?'disabled':''}>${orderSubmitting?'Оформляем…':'Подтвердить заказ'}</button>`}
        </div>
      </div>
...
function stepFulfillment(){
  ...
  <div class="checkout-block">
    ...
    ${checkoutForm.fulfillment==='pickup' ? `<p class="modal-sub">Заберите заказ по адресу: Москва, ул. Примерная, д. 10. Готовность — через 25–35 минут.</p>` : ""}
  </div>`;
}
...
function stepAddress(){
  ...
  <div class="checkout-block">
    ...
    <div class="field-grid">
      ...
      <div class="field"><label>Улица *</label><input id="f_street" type="text" value="${escapeHtml(f.street||'')}" placeholder="Улица"></div>
      <div class="field field-sm"><label>Дом *</label><input id="f_house" type="text" value="${escapeHtml(f.house||'')}" placeholder="12"></div>
      <div class="field field-sm"><label>Корпус</label><input id="f_block" type="text" value="${escapeHtml(f.block||'')}" placeholder="1"></div>
      <div class="field field-sm"><label>Квартира</label><input id="f_flat" type="text" value="${escapeHtml(f.flat||'')}" placeholder="45"></div>
      <div class="field field-sm"><label>Подъезд</label><input id="f_entrance" type="text" value="${escapeHtml(f.entrance||'')}" placeholder="2"></div>
      <div class="field field-sm"><label>Этаж</label><input id="f_floor" type="text" value="${escapeHtml(f.floor||'')}" placeholder="5"></div>
      <div class="field field-sm"><label>Домофон</label><input id="f_intercom" type="text" value="${escapeHtml(f.intercom||'')}" placeholder="45К"></div>
    </div>
    <div class="field"><label>Комментарий курьеру</label><textarea id="f_comment" rows="2" placeholder="Позвонить за 5 минут, код на воротах и т.д.">${escapeHtml(f.comment||'')}</textarea></div>
  </div>`;
}
...
function stepContacts(){
  ...
  <div class="checkout-block">
    ...
    <div class="field"><label>Имя *</label><input id="f_name" type="text" value="${escapeHtml(f.name||'')}" placeholder="Как к вам обращаться"></div>
    <div class="field"><label>Телефон *</label><input id="f_phone" type="tel" value="${escapeHtml(f.phone||'')}" placeholder="+7 (___) ___-__-__"></div>
  </div>`;
}
...
function stepPayment(){
  const f = checkoutForm;
  const opts = [
    {id:"card-online", label:"Картой онлайн", emoji:"💳"},
    {id:"cash", label:"Наличными курьеру", emoji:"💵"},
    {id:"card-courier", label:"Картой курьеру", emoji:"📲"},
  ];
  return `
  <div class="checkout-block">
    <h3>Оплата</h3>
...
function stepConfirm(){
  ...
  <div class="checkout-block">
    ...
    <div class="confirm-grid">
      <div><span>Получение</span><strong>${f.fulfillment==='delivery' ? 'Доставка' : 'Самовывоз'}</strong></div>
      ${f.fulfillment==='delivery' ? `<div><span>Адрес</span><strong>${escapeHtml(f.city)}, ${escapeHtml(f.street||'')} ${escapeHtml(f.house||'')}${f.flat? ', кв. '+escapeHtml(f.flat):''}</strong></div>` : ""}
      <div><span>Имя</span><strong>${escapeHtml(f.name||'')}</strong></div>
      <div><span>Телефон</span><strong>${escapeHtml(f.phone||'')}</strong></div>
      <div><span>Оплата</span><strong>${f.payment==='card-online'?'Картой онлайн':f.payment==='cash'?'Наличными курьеру':'Картой курьеру'}</strong></div>
    </div>
  </div>`;
}
...
function validateStep(step){
  ...
  if (step===2 && checkoutForm.fulfillment==='delivery'){
    if (!checkoutForm.street || !checkoutForm.house){ checkoutError = "Введите адрес доставки"; return false; }
  }
  if (step===3){
    if (!checkoutForm.name){ checkoutError = "Введите имя"; return false; }
    if (!checkoutForm.phone || checkoutForm.phone.replace(/\D/g,"").length < 10){ checkoutError = "Введите номер телефона"; return false; }
  }
  return true;
}
...
async function confirmOrder(){
  ...
  if (state.cart.length===0){ checkoutError = "Добавьте хотя бы один товар"; renderApp(); return; }
  if (checkoutForm.fulfillment==='delivery' && (!checkoutForm.street || !checkoutForm.house)){ checkoutError = "Введите адрес доставки"; checkoutStep=2; renderApp(); return; }
  if (!checkoutForm.name || !checkoutForm.phone){ checkoutError = "Заполните контактные данные"; checkoutStep=3; renderApp(); return; }
  ...
  try{
    ...
    state.profile.name = checkoutForm.name; state.profile.phone = checkoutForm.phone;
    state.fulfillment = checkoutForm.fulfillment;
    saveAddress(); saveProfile(); saveFulfillment();

    state.orderIds.unshift(order.id);
    ...
    orderSubmitting = false;
    checkoutError = (e.data && e.data.errors && e.data.errors.join(". ")) || e.message || "Не удалось оформить заказ. Проверьте соединение с сервером.";
    renderApp();
  }
}
...
function viewOrderSuccess(){
  ...
  if (!order) return `<section class="section"><div class="empty-state"><h3>Заказ не найден</h3><button class="btn btn-primary" data-action="go-menu">В меню</button></div></section>`;
  return `
  <section class="section order-success">
    <div class="os-card">
      <span class="os-emoji">✅</span>
      <h2>Заказ №${order.id} принят!</h2>
      <p>Мы уже готовим ваш заказ. Статус можно отслеживать в личном кабинете.</p>
      <div class="os-status">Статус: <strong>${order.status}</strong></div>
      <div class="os-total">Сумма заказа: <strong>${money(order.total)}</strong></div>
      <div class="os-actions">
        <button class="btn btn-primary" data-action="go" data-view="orders">Мои заказы</button>
        <button class="btn btn-ghost" data-action="go-menu">Вернуться в меню</button>
      </div>
    </div>
  </section>`;
}
...
function viewDelivery(){
  return `
  <section class="section">
    <div class="section-head"><h2>Доставка и оплата</h2></div>
    <div class="delivery-zones">
      ${DELIVERY_ZONES.map(z=>`
        <div class="zone-card">
          <h3>${z.name}</h3>
          <ul>
            <li><span>Минимальный заказ</span><strong>${money(z.minOrder)}</strong></li>
            <li><span>Стоимость доставки</span><strong>${z.price===0? 'Бесплатно' : money(z.price)}</strong></li>
            <li><span>Бесплатно от</span><strong>${money(z.freeFrom)}</strong></li>
            <li><span>Среднее время</span><strong>${z.time}</strong></li>
          </ul>
        </div>`).join("")}
    </div>
...
function viewAbout(){
  ...
  <section class="section about-page">
    ...
    <p class="about-lead">«Суши Pizza Тайм» — служба доставки, которая объединила два любимых города вкуса: свежие японские роллы и итальянскую пиццу на тонком тесте. Мы работаем в Москве и Московской области и каждый день собираем заказы так, будто готовим для своей семьи.</p>
    <div class="about-grid">
      <div class="about-card"><span>🌿</span><h4>Свежесть</h4><p>Рыбу и овощи закупаем ежедневно, тесто для пиццы готовим по классической рецептуре.</p></div>
      <div class="about-card"><span>⏱️</span><h4>Скорость</h4><p>Среднее время доставки по Москве — от 35 минут благодаря сети локальных кухонь.</p></div>
      <div class="about-card"><span>🤝</span><h4>Честность</h4><p>Вес и состав в карточке товара всегда соответствуют тому, что приедет в коробке.</p></div>
    </div>
  </section>`;
}
...
function viewProfile(){
  ...
  if (!p.loggedIn){
    ...
    <section class="section profile-page">
      <div class="section-head"><h2>Личный кабинет</h2></div>
      <div class="login-card">
        <span class="empty-emoji">📱</span>
        <h3>Войдите по номеру телефона</h3>
        <p>Сохраняйте адреса, повторяйте заказы и копите бонусы.</p>
        <div class="field"><label>Телефон</label><input id="loginPhone" type="tel" placeholder="+7 (___) ___-__-__" value="${escapeHtml(p.phone||'')}"></div>
        <div class="field"><label>Имя</label><input id="loginName" type="text" placeholder="Ваше имя" value="${escapeHtml(p.name||'')}"></div>
        <div id="loginError" class="field-error"></div>
        <button class="btn btn-primary btn-lg btn-block" data-action="do-login">Войти</button>
      </div>
    </section>`;
  }
  return `
  <section class="section profile-page">
    ...
    <div class="profile-card">
      ...
      <button class="btn btn-ghost" data-action="do-logout">Выйти</button>
    </div>
    <div class="profile-stats">
      <div class="stat-card"><span>${p.bonuses}</span>Бонусов</div>
      <div class="stat-card"><span>${state.orderIds.length}</span>Заказов</div>
      <div class="stat-card"><span>${state.favorites.length}</span>В избранном</div>
    </div>
    <div class="profile-links">
      <button class="profile-link" data-action="go" data-view="orders">📦 История заказов</button>
      <button class="profile-link" data-action="go" data-view="favorites">❤️ Любимые блюда</button>
      <button class="profile-link" data-action="edit-address">📍 Сохранённый адрес: ${escapeHtml(state.address.street ? state.address.city+', '+state.address.street : state.address.city)}</button>
    </div>
  </section>`;
}

// ---------------------------------------------------------------------------
// VIEW: ORDERS (список ID в localStorage, данные всегда актуальны с сервера)
// ---------------------------------------------------------------------------
function viewOrders(){
  if (state.orderIds.length===0){
    return `<section class="section"><div class="empty-state"><span class="empty-emoji">📦</span><h3>Заказов пока нет</h3><p>Оформите первый заказ — он появится здесь.</p><button class="btn btn-primary" data-action="go-menu">Перейти в меню</button></div></section>`;
  }
  return `
  <section class="section">
    <div class="section-head"><h2>История заказов</h2></div>
    <div class="orders-list">
      ${state.orderIds.map(id=>{
        const o = state.ordersCache[id];
        if (!o){
          return `<div class="order-card order-card-loading">Загружаем заказ ${id}…</div>`;
        }
        return `
        <div class="order-card">
          ...
          <div class="order-card-foot">
            ...
            <button class="btn btn-ghost btn-sm" data-action="repeat-order" data-id="${o.id}">Повторить заказ</button>
          </div>
        </div>`;
      }).join("")}
    </div>
...
async function afterRenderOrders(){
  const results = await Promise.allSettled(state.orderIds.map(id => apiGet(`/api/orders/${id}`)));
  let changed = false;
  results.forEach((r, idx) => {
    if (r.status === "fulfilled"){ state.ordersCache[state.orderIds[idx]] = r.value; changed = true; }
  });
  if (changed && currentView()==="orders") renderApp();
}

function repeatOrder(orderId){
  const order = state.ordersCache[orderId];
...
document.addEventListener("click", (e)=>{
  ...
  switch(action){
    ...
    case "do-logout": state.profile.loggedIn = false; saveProfile(); renderApp(); break;
...
async function applyPromoCode(code){
  ...
  try{
    ...
    if (!result.valid){
      if (errEl) errEl.textContent = result.message || "Промокод недействителен";
      return;
    }
...
function doLogin(){
  ...
  if (!phone || phone.replace(/\D/g,"").length < 10){ err.textContent = "Введите номер телефона"; return; }
  if (!name){ err.textContent = "Введите имя"; return; }
  state.profile = { phone, name, loggedIn:true, bonuses: state.profile.bonuses||150 };
  saveProfile();
  toast("Вы вошли в личный кабинет", "success");
  renderApp();
}
...
async function boot(){
  ...
  try{
    ...
    text.innerHTML = `Не удалось подключиться к серверу.<br>Убедитесь, что бэкенд запущен (<code>node server.js</code>) и страница открыта с того же адреса (например http://localhost:4000).<br><br><button class="btn btn-primary loading-retry" onclick="location.reload()">Повторить попытку</button>`;
  }
}
boot();
